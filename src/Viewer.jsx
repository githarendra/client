...
export default function Viewer() {
    ...
    myPeer.current.on('call', (call) => {
      ...
      call.on('stream', (hostStream) => {
        if(videoRef.current) {
            ...
            const attemptPlay = () => {
                videoRef.current.play().catch(() => setTimeout(attemptPlay, 500));
            };
            attemptPlay();
  ...
  const handleManualPlay = () => {
    ...
    if (type === 'PAUSE') {
        ...
        setIsPaused(true);
        setStatus("Host Paused");
        isLocallyPaused.current = false; // Add this line if needed
        socket.emit('viewer-status-update', { roomId, status: 'PAUSE' });
    } else {
        setIsPaused(false);
        setStatus("LIVE");
          ...
          <div className="flex h-screen w-screen bg-black items-center justify-center font-sans relative">
              ...
              <form onSubmit={handleLogin} className="z-10 bg-zinc-900/80 backdrop-blur-xl p-10 rounded-3xl border border-white/10 flex flex-col gap-6 w-96 shadow-2xl relative overflow-hidden">
                  ...
                  <div className="text-center">
                      ...
                      <p className="text-zinc-500 text-sm">Room: <span className="font-mono text-zinc-300">{roomId}</span></p>
                  </div>
                  <div className="flex flex-col gap-2 text-left">
                      <label className="text-zinc-400 text-xs uppercase tracking-wide font-bold ml-1">Your Name</label>
                      <input ref={nameInputRef} type="text" placeholder="e.g. Viewer Vinny" className="bg-black/50 border border-zinc-700 text-white px-4 py-3 rounded-xl focus:border-blue-500 focus:ring-1 focus:ring-blue-500 outline-none transition placeholder-zinc-600" autoFocus />
                  </div>
  ...
  return (
    <div className="flex flex-col h-screen w-screen bg-black overflow-hidden font-sans">
      <div className="h-14 flex items-center justify-between px-6 bg-zinc-950/80 backdrop-blur-md border-b border-white/5 shrink-0 z-20">
         ...
         <div className="flex items-center gap-3">
             ...
             <div className="px-3 py-1.5 bg-black/40 border border-white/5 rounded-full flex items-center gap-2">
                <div className={`w-2 h-2 rounded-full ${isEnded ? 'bg-red-500' : status === 'LIVE' ? 'bg-green-500 animate-pulse' : 'bg-yellow-500'}`}></div>
      ...
      <div className="flex-1 min-h-0 flex flex-row relative overflow-hidden bg-black">
        <div className="flex-1 flex items-center justify-center relative min-w-0">
          ...
          {isPaused && !isEnded && !showPlayButton && (
            <div className="absolute inset-0 flex items-center justify-center pointer-events-none"><div className="bg-black/60 p-8 rounded-full backdrop-blur-md border border-white/10"><svg className="w-16 h-16 text-white/90 fill-current" viewBox="0 0 24 24"><path d="M6 19h4V5H6v14zm8-14v14h4V5h-4z"/></svg></div></div>
          )}
          {isEnded && (
            <div className="absolute inset-0 z-[100] flex items-center justify-center bg-zinc-950">
                <div className="text-center p-12 border border-zinc-800 rounded-3xl bg-black shadow-2xl">
                    <div className="text-6xl mb-6 grayscale opacity-50">📺</div>
                    <h1 className="text-2xl font-bold text-zinc-300 mb-2">Host Offline</h1>
                    <p className="text-zinc-600">Waiting for signal...</p>
                </div>
            </div>
          )}
        </div>
