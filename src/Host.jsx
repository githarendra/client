...
export default function Host() {
  ...
  useEffect(() => {
    ...
    // ✅ 2. Receive User List from Server
    socket.on('update-user-list', (updatedUsers) => {
        setUsers(updatedUsers);
    });

    // ✅ 3. New Viewer asking for Sync? Reply instantly!
    socket.on('send-sync-to-new-viewer', (viewerSocketId) => {
        if(videoRef.current) {
    ...
    // ✅ 4. PeerJS: Connect stream to new users
    ...
    // Chat
    const handleMessage = (data) => setMessages((prev) => [...prev, { ...data, isMe: false }]);
    socket.on('receive-message', handleMessage);

    return () => {
        socket.off('update-user-list');
        socket.off('send-sync-to-new-viewer');
  ...
  const handleKick = (socketId, name) => {
      if(window.confirm(`Kick ${name}?`)) socket.emit('kick-user', { roomId, socketId });
  };
  ...
  const startBroadcast = async () => {
    ...
    try { 
        ...
        socket.emit('host-joined', { roomId, username }); // Re-confirm host
  ...
  if (!isLoggedIn) {
      return (
          <div className="flex h-screen w-screen bg-black items-center justify-center font-sans relative">
              ...
              <form onSubmit={handleLogin} className="z-10 bg-zinc-900/80 backdrop-blur-xl p-10 rounded-3xl border border-white/10 flex flex-col gap-6 w-96 shadow-2xl relative">
                  ...
                  <button type="submit" className="bg-violet-600 hover:bg-violet-500 text-white font-bold py-3.5 rounded-xl transition-all hover:scale-[1.02] shadow-lg">Start Hosting</button>
              </form>
          </div>
      );
  }
  ...
  return (
    <div className="flex flex-col h-screen w-screen bg-black text-white overflow-hidden font-sans">
      <div className="h-16 flex items-center justify-between px-6 bg-zinc-950/80 backdrop-blur-md border-b border-white/5 shrink-0 z-20">
        <div className="flex items-center gap-6"><div className="flex items-center gap-3 group"><div className="w-8 h-8 bg-violet-600 rounded-lg flex items-center justify-center font-bold text-white">P</div><h1 className="text-lg font-bold tracking-tight text-zinc-200">Party<span className="text-violet-500">Time</span></h1></div><div className="h-6 w-px bg-white/10 hidden md:block"></div><div className="flex flex-col"><span className="text-[10px] text-zinc-500 font-bold uppercase tracking-widest">Room ID</span><span className="text-sm font-mono text-zinc-300 leading-none">{roomId}</span></div></div>
                    ...
                    {users.map(u => (
                        <div key={u.socketId} className="flex items-center justify-between p-2.5 rounded-xl hover:bg-white/5 transition group">
                            <div className="flex items-center gap-3">
                                ...
                                <div className="flex flex-col"><span className="font-bold text-sm text-zinc-200">{u.username}</span><span className={`text-[10px] font-bold uppercase ${u.status === 'LIVE' ? 'text-green-500' : 'text-yellow-500'}`}>{u.status === 'LIVE' ? '▶ Watching' : '⏸ Paused'}</span></div>
                            </div>
      ...
      <div className="h-24 flex items-center justify-center gap-6 bg-zinc-950 border-t border-white/5 shrink-0 z-50">
        <label className={`cursor-pointer group flex items-center justify-center gap-3 w-80 h-16 bg-zinc-900 border border-zinc-800 rounded-2xl hover:bg-zinc-800 hover:border-zinc-700 transition shadow-lg ${isBroadcasting ? 'opacity-50 pointer-events-none' : ''}`}>
            ...
            <div className="flex flex-col items-start"><span className="text-[10px] text-zinc-500 font-bold uppercase tracking-wider">Source Media</span><span className="text-sm font-bold text-white truncate max-w-[150px]">{fileSelected ? "Video Loaded" : "Select File"}</span></div>
