...
export default function Chat({ socket, roomId, toggleChat, username, messages, setMessages }) {
  ...
  return (
    <div className="flex flex-col h-full w-80 bg-black/60 backdrop-blur-xl border-l border-white/10 shadow-2xl z-40 relative shrink-0 transition-all duration-300 ease-in-out">
      ...
      {/* Messages */}
      <div className="flex-1 overflow-y-auto p-4 space-y-4 scrollbar-thin scrollbar-thumb-zinc-800 scrollbar-track-transparent relative">
        {/* ✅ NEW: Empty State UI */}
        {messages.length === 0 && (
            <div className="absolute inset-0 flex items-center justify-center opacity-30 select-none pointer-events-none">
                <div className="text-center">
                    <span className="text-4xl block mb-2">👋</span>
                    <span className="text-sm font-bold text-white uppercase tracking-widest">Say Hello!</span>
                </div>
            </div>
        )}
